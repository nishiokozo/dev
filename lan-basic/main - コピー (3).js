...
//-----------------------------------------------------------------------------
{
	for ( let key of Object.keys(g_tblVarobj) )
	{
		let v = g_tblVarobj[key];
...
class Token
{
	...
	constructor( strProg, tblVarobj )
	{
		this.adr = 0;
		this.str = strProg;
		this.tblVarobj = tblVarobj;
...
//			oc_dump();
			...
			if ( oc.length == 4 && oc[0].val == "if" && oc[1].atr == "N:" && oc[2].atr == "P:" && oc[3].val != "else" )
			{
				...
				if ( cond )
				{
					let token = new Token( prog.val, this.tblVarobj );
			...
			if ( oc.length == 5 && oc[0].val == "if" && oc[1].atr == "N:" && oc[2].atr == "P:" && oc[3].val == "else" && oc[4].atr == "P:" )
			{
				...
				if ( cond.val )
				{
					let token = new Token( prog1.val, this.tblVarobj );
					token.execScript( _if.val.val );	//
				}
				else
				{
					let token = new Token( prog2.val, this.tblVarobj );
			...
			if ( oc.length == 3 && oc[0].atr == "T:" && oc[1].atr == "W:" && oc[2].atr != "T:"  )
			{
				...
				this.tblVarobj[ name.val ] = {val:0, atr:type.val};
			...
			if ( oc.length == 4 && oc[0].atr == "T:" && oc[1].atr == "W:" && oc[2].atr == "T:" && oc[3].atr == "P:" )
			{
				...
				this.tblVarobj[ name.val ] = {val:prog.val, atr:type.val, arg:arg} ;
			...
			if ( oc.length == 2 && oc[0].atr == "W:" && oc[1].atr == "A:" )
			{
				...
				let param	= oc.pop();	//	[]
				let	name	= oc.pop();	//	foo
				let prog	= this.tblVarobj[ name.val ].val;
				let arg		= this.tblVarobj[ name.val ].arg;
...
//					p.val
					...
					this.tblVarobj[ a.name ] = {val:p.val, atr:"F:"};

				}


				{
					let token = new Token( prog, this.tblVarobj );
