...
//-----------------------------------------------------------------------------
{
	for ( let v of Object.keys(g_tblVarval) )
	{
		do_delAfter();

		if ( g_tblVararg[v] != undefined )
		{
			cmd_print( g_tblVaratr[v]+" "+v+" : "+g_tblVarval[v] +":" );

			let arg = g_tblVararg[v];
			cmd_print( "(" );
			for ( let a of arg )
			{
				cmd_print( a.atr );
...
//	(atr) function array[n]
	...
	//	S:	Seperator Object
		...
		//-----------------------------------------------------------------------------
		{
			...
			if ( undefined != g_tblVarval[ name ] )
			{
				name = g_tblVarval[ name ];		// 変数取り出し
			}
...
//			oc_dump();
						...
						case "=":	
							{
								let val = v2.val;						
								let atr = v2.atr;						
								if ( undefined != g_tblVarval[ val ] )		// 変数取り出し
								{
									atr = g_tblVaratr[ val ];
									val = g_tblVarval[ val ];
								}

								if ( undefined != g_tblVarval[ v1.val ] )	// 変数代入
								{
									if ( g_tblVaratr[ v1.val ] == atr )
									{
										g_tblVarval[ v1.val ] = val;	
									}
									else
									{
			...
			if ( oc.length == 3 && oc[0].atr == "W:" && oc[1].atr == "W:" && oc[2].atr != "T:"  )
			{
				console.log("WW~T:変数定義");
				let	o1		= oc.pop();	//	~A:
				let	name	= oc.pop();	//	a
				let type	= oc.pop();	//	float

				g_tblVarval[ name.val ] = 0;
				g_tblVaratr[ name.val ] = type.val;

			}
			if ( oc.length == 4 && oc[0].atr == "W:" && oc[1].atr == "W:" && oc[2].atr == "T:" && oc[3].atr == "P:" )
			{
				console.log("WWTP:関数定義");
				let prog	= oc.pop();	//	<>
				let form	= oc.pop();	//	()	
				...
				g_tblVarval[ name.val ] = prog.val;
				g_tblVaratr[ name.val ] = type.val;
				g_tblVararg[ name.val ] = arg;
			...
			if ( oc.length == 2 && oc[0].atr == "W:" && oc[1].atr == "A:" )
			{
				...
				let param	= oc.pop();	//	{}
				let	name	= oc.pop();	//	_foo
				let prog	= g_tblVarval[ name.val ];
				let arg		= g_tblVararg[ name.val ];


				console.log( param );
				//for ( let a of param.val )
				for ( let i = 0 ; i < param.val.length && i < arg.length ; i++ )
				{
					let p = param.val[i];
					let a = arg[i];

console.log( ">>>",a.name, p.val );
					g_tblVarval[ a.name ] = p.val;
					g_tblVaratr[ a.name ] = "F:";
