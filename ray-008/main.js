...
//------------------------------------------------------------------------------
{
	...
	case "4balls":
		{
			g_tblPlate.push( new Plate( P=new vec3( 0  ,  0 ,0.0),N=new vec3(0,1,0),C=new vec3(0.8,0.8,0.8),rl=0.5,rr=1.0 ,pw=20,e= 0.0,tm=0.0 ) );
			g_tblSphere.push( new Sphere(new vec3( 0.0 , 1.25, 0       ),   0.5 , new vec3(1  , 0.2, 0.2), 0.5, 1.0, 20, 0.0, 0.0 ) );
			g_tblSphere.push( new Sphere(new vec3( 0.0 , 0.5 , -0.433 ),   0.5 , new vec3(0.0, 0.0, 0.0), 1.0, 1.0, 20, 0.0, 0.0 ) );
			g_tblSphere.push( new Sphere(new vec3( 0.5 , 0.5 , +0.433 ),   0.5 , new vec3(0.2, 0.2, 1.0), 0.5, 1.0, 20, 0.0, 0.0 ) );
			g_tblSphere.push( new Sphere(new vec3(-0.5 , 0.5 , +0.433 ),   0.5 , new vec3(0.0, 1.0, 0.0), 0.5, 1.0, 20, 0.0, 0.0 ) );
...
//			g_tblSphere.push( new Sphere(new vec3(-0.0 , 1.0 , 8 ),   1.0 , new vec3(0.1, 1.0, 0.1) , 0.0, 0.0, 20, 0.0, 0.0 ) );
			...
			g_tblSphere.push( new Sphere(new vec3(-1.0 , 1.0 ,-2),   1.0 , new vec3(0.7, 0.7, 0.7), 0.0, 0.96, 20, 0.0, 0.5 ) );
			g_tblSphere.push( new Sphere(new vec3( 1.0 , 1.0 ,+2),   1.0 , new vec3(0.7, 0.7, 0.7), 0.0, 0.5, 20, 0.0, 0.5 ) );
...
//console.log(ry/3.14*180, posEye.x-posAt.x, posEye.z-posAt.z);
...
//------------------------------------------------------------------------------
{
	...
	// javascript側で初期のキャンバスサイズを決める
	{
		...
		html_canvas.height = html_canvas.width/2;
...
//-----------------------------------------------------------------------------
{
	...
	let [rx,ry] = a( vsub(posAt, posEye) ); 
...

		{
			...
			if ( c == KEY_UP	) {posAt.y +=s;}
			if ( c == KEY_DOWN	) {posAt.y -=s;}
			if ( c == KEY_LEFT	) {dir=rad(0);spd=s;}
			if ( c == KEY_RIGHT	) {dir=rad(180);spd=s;}

			if ( c == KEY_W	) {dir=rad( 90);spd=s;	dir2=rad( 90);spd2=s;}
			if ( c == KEY_S	) {dir=rad(-90);spd=s;	dir2=rad(-90);spd2=s;}
			if ( c == KEY_D	) {dir=rad(  0);spd=s;	dir2=rad(  0);spd2=s;}
			if ( c == KEY_A	) {dir=rad(180);spd=s;	dir2=rad(180);spd2=s;}
			if ( c == KEY_R	) {posEye.y+=s;posAt.y +=s;}
			if ( c == KEY_F	) {posEye.y-=s;posAt.y -=s;}
			if ( c == KEY_Q	) {rz+=rad(2);}
			if ( c == KEY_E	) {rz-=rad(2);}

			if ( spd > 0 )
			{
				posEye.x += Math.cos( ry+dir )*spd;
				posEye.z += Math.sin( ry+dir )*spd;
			}
			
			if ( spd2 > 0 )
			{
				posAt.x += Math.cos( ry+dir2 )*spd2;
				posAt.z += Math.sin( ry+dir2 )*spd2;
			}
		}

	document.getElementById( "html_eye_x" ).value = posEye.x;
	document.getElementById( "html_eye_y" ).value = posEye.y;
	document.getElementById( "html_eye_z" ).value = posEye.z;
	document.getElementById( "html_at_x" ).value = posAt.x;
	document.getElementById( "html_at_y" ).value = posAt.y;
	document.getElementById( "html_at_z" ).value = posAt.z;
	document.getElementById( "html_fovy" ).value = fovy * 180 /Math.PI;
	document.getElementById( "html_rz" ).value = rz * 180 /Math.PI;
