...
/*
					...
					"制御を失った宇宙ステーションの落下が始まり脱出が必要となった・・・w10t01",
					"w30",
					"人がいる。",
					"「おーいハヤト！」",
					"あなたを呼んでいる。",
...
//-----------------------------------------------------------------------------
{
	...
	else
	{
		while ( message.tic < message.str.length )
		{
			...
			{
				let cmd = message.str.substr( message.tic, 4 );
				if ( cmd[0] == "j" )
				{
					g_nextScene = cmd.substr(1,3);
					message.tic+="j###".length;
				}
				else
				if ( cmd[0] == "t" )
				{
					let val = Number( cmd.substr(1,2) );
